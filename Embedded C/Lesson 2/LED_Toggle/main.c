...
#define RCC_APB2ENR *(vuint32_t*)(RCC_BASE   + 0x18)       //ADDRESS OF ENABLE REGISTER OF PORTA
#define GPIOA_CHR   *(vuint32_t*)(GPIOA_BASE + 0x04)       //ADDRESS OF CHR REGISTER OF PORTA
#define GPIOA_ODR   *(vuint32_t*)(GPIOA_BASE + 0x0C)       //ADDRESS OF ODR REGISTER OF PORTA

typedef union{
	      vuint32_t  ALL_fileds;                             //ALL_fileds : to access the whole register
	     struct{
					vuint32_t  reserved:13;                          //don't need first 12 bits, so reserved
					vuint32_t  p_13:1;                               //p_13: access bit no.13 
				  }pin;
}R_ODR_t;
